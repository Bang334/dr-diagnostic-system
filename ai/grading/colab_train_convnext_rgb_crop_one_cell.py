# %%
# ONE COLAB CELL: improve ConvNeXt-Tiny from its best RGB-crop checkpoint.
...
SOURCE_RUN_DIR = Path("/content/drive/MyDrive/dr_runs/convnext_rgb_crop")
OUTPUT_DIR = Path("/content/drive/MyDrive/dr_runs/convnext_rgb_crop_improved")
...
RESUME_CHECKPOINT = (
    ...
    if (OUTPUT_DIR / "checkpoint-last.pth").exists()
        ...
        if (OUTPUT_DIR / "checkpoint-best.pth").exists()
...
def evaluate(loader):
    ...
    for images, targets in tqdm(loader, desc="Evaluate", leave=False):
        ...
        with torch.amp.autocast("cuda", enabled=amp_enabled):
            logits = model(images)
        loss, _, _ = combined_loss(logits, targets)
...
if RESUME_TRAINING and RESUME_CHECKPOINT.exists():
    ...
    raise FileNotFoundError(
        f"Cannot continue ConvNeXt because checkpoint was not found: "
        f"{RESUME_CHECKPOINT}"
    )
...
pd.DataFrame(classification).transpose().to_csv(
...
for grade, class_name in enumerate(CLASS_NAMES):
    grade_mask = targets_array == grade
    total = int(grade_mask.sum())
    correct = int((predictions_array[grade_mask] == grade).sum())
    per_grade_rows.append(
        {
            ...
            "recall_percent": round(
                100.0 * correct / total if total else 0.0,
                2,
            ),
        }
    )
per_grade_table = pd.DataFrame(per_grade_rows)
per_grade_table.to_csv(OUTPUT_DIR / "test_per_grade.csv", index=False)
