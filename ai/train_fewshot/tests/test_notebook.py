...
class FewShotNotebookTests(unittest.TestCase):
    ...
    def test_downloads_dataset_to_colab_local_storage(self):
        self.assertIn("DEEPDRID_RAW = Path('/content/dr_raw')", self.source)
        self.assertIn("DEEPDRID_PREPARED = Path('/content/dr')", self.source)
        self.assertIn("shutil.unpack_archive", self.source)
        self.assertNotIn("datasets", self.source.lower())
    ...
    def test_streams_to_console_without_creating_notebook_logs(self):
        ...
        self.assertNotIn("_notebook_logs", self.source)
        self.assertNotIn("log_path", self.source)
